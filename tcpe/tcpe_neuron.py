...
def apply_tcpe_neuron(
    ...
    """
    Updating weights based on the provided index and update tensor files.

    :param copy: If true, preserves the original model while creating a new one to edit.
    :param return_orig_weights: If true, returns the original weights that were changed.
    :return: Updated model, and a copy of original weights that changed.
    """
    if copy:
    ...
    # Load k_star related update_matrix or complete update_matrix
    if lim >= 0:
        print("=====Load k_star related update_matrix=====")
        update_matrix, kstar_num = load_update_matrix(abla_exp, index_file_path, value_file_path, upd_file_path, lim)
    ...
    with torch.no_grad():
        ...
        print(">>> upd_decay:", upd_decay)

    return model, weights_copy



def load_update_matrix(abla_exp: bool, index_file_path: str, value_file_path: str, upd_file_path: str, lim: float) -> torch.Tensor:
    ...
    # Filter INDEX based on VALUE (Example: mask for VALUE >= 1e-01)
    mask = VALUE >= lim
