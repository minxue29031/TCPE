...
TREE_SITTER_ROOT = Path(__file__).parents[0].absolute().joinpath("tree-sitter-cache")
NEW_LINE = "NEW_LINE"
...
class TreeSitterLangProcessor(LangProcessor):
    ...
    def ensure_treesitter_installed(self, repo_path: str):
        ...
        'TREESITTER_REPOSITORY'. 'TREESITTER_REPOSITORY' is in the form of
        e.g.: 'tree-sitter/tree-sitter-python'. This allows custom Treesitter
        parsers.
        """
        if repo_path.exists():
            return
        logger.info(
            f"Treesitter repository for language {self.language} could"
    ...
    def tokenize_code(self, code, keep_comments=False, process_strings=True):
        ...
        for token, token_type in zip(tokens, token_types):
            if skip_next_new_line and token == "NEW_LINE":
                continue
    ...
    def detokenize_code(self, code):
        ...
        replaced_tokens = []
        # call parser of the tokenizer to find comments and string
        # and detokenize them correctly
        try:
            tokens, token_types = self.get_tokens_and_types(code)
            for token, token_type in zip(tokens, token_types):
                if token_type in self.ast_nodes_type_string:
            ...
            pass  # TODO: get exact exception type ignored
    ...
    def extract_arguments_using_parentheses(self, function):
        ...
        par = 0
        arguments = []
        function = function[function.index("(") :]  # noqa: E203
        for tok in function:
            if tok == "(":
                par += 1
            elif tok == ")":
    ...
    def has_class_functions(self):
        return hasattr(self.__class__, "NO_CLASS_FUNCTIONS")
