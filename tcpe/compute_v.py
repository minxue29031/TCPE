...
def compute_v(
    ...
    wandb.log({
        ...
        "Pre-target norm": target_init.norm().item(),
        "Current-target norm": target.norm().item(),
        "Change in target norm": (target.norm() - target_init.norm()).item(),
        "Division Factor": torch.dot(k_star, left_vector).item(),
        "Right vector norm": right_vector.norm()
    })  
