...
def load_model(path, device_map=None):
    if device_map is None:
        device_map = "cuda"
    return AutoModelForCausalLM.from_pretrained("/nfs/data/projects/minxue/models/model", torch_dtype=torch.float16, device_map=device_map)


def load_tokenizer(path):
    return AutoTokenizer.from_pretrained("/nfs/data/projects/minxue/models/model")
...
def load_model_and_tokenizer_with_TC_layer(
    ...
    model = load_model("/nfs/data/projects/minxue/models/model", device_map=device_map)
    ...
    tok = load_tokenizer("/nfs/data/projects/minxue/models/model")
    if tok.pad_token is None:
        tok.add_special_tokens({"pad_token": "<pad>"})
        model.config.pad_token_id = tok.pad_token_id
        model.model.padding_idx = model.config.pad_token_id
