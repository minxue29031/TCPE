...
from tcpe.tcpe_pre import tcpe_precompute
from tcpe.tcpe_multineuron import apply_tcpe
...
def demo_model_editing(
    ...
    lim: Union[float, List[float]],
    upd_decay_list: Union[float, List[float]],
    save_path_list: Optional[Union[str, List[str]]] = None,
    ...
    # Apply model editing and perform translation based on the selected algorithm
    if alg_name == "TCPE_PRE":
        # Apply ROME editing and save original weights
        print_loud("Computing all neuron uodated using ROME..")
        model_new, orig_weights = tcpe_precompute(
            model, tok, requests, hparams, save_k_v_upd, save_path, return_orig_weights=True
        )
        
        # Perform translation after ROME_PURE model editing
        print_loud("Generating translations using Codellama & KE...")
        ...
        model_new, orig_weights = apply_tcpe(
            model, tok, requests, hparams, abla_exp, save_path_list, upd_decay_list, lim, return_orig_weights=True
        )
        
        # Perform translation after TCPE model editing
        print_loud("Generating translations using Codellama & KE_KSTAR...")
        translating_TC.generate_translations(
            model_new, tok, d_params_path, originals_path, output_folder_path, translation_batch_size
        )
        return model_new, orig_weights
